...
export interface V2NearMiss {
  ...
  outcome_won: 0 | 1 | null;
  outcome_checked_at: number | null;
  created_at: number;
}

export interface V2NearMissRuleStat {
  rule: string;
  count: number;
  won_count: number;
...
export interface V2NearMissCoinStat {
  coin: string;
  count: number;
  won_count: number;
  checked_count: number;
...
export interface V2NearMissAnalysis {
  total: number;
  byRule: V2NearMissRuleStat[];
  byCoin: V2NearMissCoinStat[];
  byBlockReason: { reason: string; count: number; won_count: number; checked_count: number }[];
  recommendations: string[];
}

export async function apiV2NearMisses(opts: { sessionId?: string; coin?: string; rule?: string; page?: number; limit?: number } = {}): Promise<{ near_misses: V2NearMiss[]; total: number }> {
  const p = new URLSearchParams();
  if (opts.sessionId) p.set("sessionId", opts.sessionId);
  if (opts.coin) p.set("coin", opts.coin);
  if (opts.rule) p.set("rule", opts.rule);
  if (opts.page) p.set("page", String(opts.page));
  if (opts.limit) p.set("limit", String(opts.limit));
  const res = await fetch(`${base()}/api/v2/near-misses?${p.toString()}`);
  if (!res.ok) throw new Error(await res.text());
  return res.json();
}

export async function apiV2NearMissAnalysis(): Promise<V2NearMissAnalysis> {
  const res = await fetch(`${base()}/api/v2/near-misses/analysis`);
  if (!res.ok) throw new Error(await res.text());
  return res.json();
}
