...
export function FilterBar({ filters, onChange }: FilterBarProps) {
  const [meta, setMeta] = useState<V2Filters>({ agents: [], operators: [], coins: [], modes: [], rules: [], activeAgents: [] });
      ...
      {meta.activeAgents.length > 0 && (
        <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", flexWrap: "wrap" }}>
          ...
          <span style={{ fontSize: "0.75rem", color: "#4ade80" }}>
            {meta.activeAgents.length === 1 ? "1 agent live:" : `${meta.activeAgents.length} agents live:`}
          </span>
          {meta.activeAgents.map((a) => (
            <button
              key={a}
              onClick={() => set({ agent: a })}
              style={{
