...
import { apiGetClosedPositions, apiHlTradeUsernames, apiHlTrades, apiGetSessions, type HlTrade, type SessionRow } from "../lib/api";
...
export function PnLPage() {
  ...
  // Live sessions state
  const [liveUsernames, setLiveUsernames] = useState<string[]>([]);
  const [sessionModes, setSessionModes] = useState<Map<string, "live" | "simulated">>(new Map());
  ...
  useEffect(() => {
    if (tab !== "live") return;
    setLiveLoading(true);
    Promise.all([apiHlTradeUsernames(), apiGetSessions(200)])
      .then(([usernames, sessions]: [string[], SessionRow[]]) => {
        setLiveUsernames(usernames);
        const modeMap = new Map<string, "live" | "simulated">();
        sessions.forEach((s) => modeMap.set(s.id, s.mode));
        setSessionModes(modeMap);
      })
      .catch(() => setLiveUsernames([]))
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", background: "#1e293b", borderRadius: "8px", overflow: "hidden" }}>
              ...
              <tbody>
                {liveUsernames.map((username) => {
                  ...
                  const mode = sessionModes.get(username);
                  return (
                    <Fragment key={username}>
                      ...
                      >
                        <td style={{ ...tdStyle, fontFamily: "monospace", fontSize: "0.85rem", maxWidth: 260, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                          {username}
                        </td>
                        <td style={tdStyle}>
                          {mode ? (
