...
echo "    npx tsx hyperliquid-trader/src/agent.ts             — live trade"
echo ""
echo "  Add erde to your PATH — paste this into ~/.zshrc or ~/.bashrc:"
echo "    export PATH=\"\$PATH:$(pwd)/bin\""
echo ""
echo "  Then start with: erde"
echo ""
...
if command -v claude &>/dev/null; then
  ...
  echo ""
  sleep 1
  exec claude "hl-trader setup is complete and the trading profile has been saved. Use your Bash tool to do these two things right now: (1) Start the web dashboard in the background: run 'npm run server'. (2) Start the trading agent in dry-run mode: run 'npx tsx hyperliquid-trader/src/agent.ts --dry-run --no-vol-detect'. Launch both now, show me the startup output, and confirm when they are running."
else
