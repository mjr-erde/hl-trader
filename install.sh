...
echo "    npm run server        — web dashboard on :3000"
echo "    npx tsx hyperliquid-trader/src/agent.ts --help-me   — trading wizard"
echo "    npx tsx hyperliquid-trader/src/agent.ts --dry-run   — paper trade"
echo ""
...
if command -v claude &>/dev/null; then
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
  echo ""
  bold "Launching Claude to configure your trading agent..."
  echo ""
  sleep 1
  exec claude --print "Welcome to hl-trader! Setup is complete.

Please help the user configure and launch a new trading agent session:

1. Run the trading wizard interactively:
   npx tsx hyperliquid-trader/src/agent.ts --help-me
   (This asks about risk appetite, strategy, and coin selection — then saves a profile)

2. After the wizard, start the web dashboard in the background:
   npm run server

3. Then launch the agent in live or dry-run mode based on what the user wants.

Ask the user if they want to do a dry-run first (recommended) or go straight to live trading.
Also check if they have their Hyperliquid private key set in hyperliquid-trader/.env — if not, help them add it before starting."
else
  warn "Claude CLI not found — install it with: npm install -g @anthropic-ai/claude-code"
  echo ""
  echo "  Once installed, run: claude ."
  echo "  Then ask Claude to help you configure a new trading agent."
  echo ""
fi
