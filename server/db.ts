...
function initSchema(db: Database.Database): void {
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_near_misses_session ON near_misses(session_id);
    CREATE INDEX IF NOT EXISTS idx_near_misses_created ON near_misses(created_at DESC);
    CREATE INDEX IF NOT EXISTS idx_near_misses_rule    ON near_misses(rule);
    CREATE INDEX IF NOT EXISTS idx_near_misses_coin    ON near_misses(coin);
  ...
  safeAlter(db, "sessions", "operator", "TEXT");
  safeAlter(db, "sessions", "last_heartbeat", "INTEGER");

  // New indexes for v2 filter queries
  try {
    db.exec(`
      CREATE INDEX IF NOT EXISTS idx_trades_filter
        ON trades(opened_at DESC, marketplace, mode, coin);
      CREATE INDEX IF NOT EXISTS idx_sessions_operator
        ON sessions(operator, started_at DESC);
    `);
  } catch { /* non-fatal */ }
