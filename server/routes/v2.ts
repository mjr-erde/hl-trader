 ...
 *   GET /api/v2/pnl/by-session  — P&L aggregated per session
...
function buildTradeFilter(query: Record<string, string | undefined>) {
  const { agent, operator, from, to, coin, side, mode, marketplace, rule } = query;
...
router.get("/trades/export", (req, res) => {
  ...
  try {
    ...
    const rows = db.prepare(`
      SELECT
        t.id, t.session_id, t.coin, t.side, t.entry_price, t.exit_price,
        t.size, t.leverage, t.strategy_reason, t.mode, t.marketplace,
        t.opened_at, t.closed_at, t.realized_pnl, t.fee, t.comment,
        s.operator
      FROM trades t
      LEFT JOIN sessions s ON t.session_id = s.id
      ${where}
...
router.get("/filters", (req, res) => {
  try {
    ...
    const agents = (db.prepare(
      "SELECT DISTINCT session_id as v FROM trades ORDER BY v"
    ...
    const coins = (db.prepare(
      "SELECT DISTINCT coin as v FROM trades ORDER BY v"
    ).all() as { v: string }[]).map((r) => r.v);

    const modes = (db.prepare(
      "SELECT DISTINCT mode as v FROM trades ORDER BY v"
    ).all() as { v: string }[]).map((r) => r.v);

    const rules = (db.prepare(
      `SELECT DISTINCT strategy_reason as v FROM trades
       WHERE strategy_reason IS NOT NULL ORDER BY v`
    ).all() as { v: string }[]).map((r) => r.v);

    res.json({ agents, operators, coins, modes, rules });
  } catch (e) {
    res.status(500).json({ error: (e as Error).message });
  }
});
