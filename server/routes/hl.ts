...
router.get("/positions", async (req, res) => {
  ...
  if (!account) {
    res.status(503).json({
      error: "no_wallet",
      message: "No wallet configured — set HYPERLIQUID_ACCOUNT_ADDRESS in hyperliquid-trader/.env to view live positions",
    });
    return;
  }
  try {
...
router.get("/closed-positions", async (req, res) => {
  ...
  if (!account) {
    res.status(503).json({
      error: "no_wallet",
      message: "No wallet configured — set HYPERLIQUID_ACCOUNT_ADDRESS in hyperliquid-trader/.env",
    });
    return;
  }
  try {
