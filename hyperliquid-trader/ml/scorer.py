...
def build_features(row: dict, coin_encoder: dict) -> list:
    """Build 21-element feature vector from a training or scoring row.

    Features:
      0  adx              – ADX strength
      1  plus_di          – +DI value
      2  minus_di         – –DI value
      3  di_spread        – signed DI spread (positive = trade direction)
      4  rsi              – RSI(14)
      5  macd_hist        – MACD histogram
      6  macd_line        – MACD line
      7  bb_width         – Bollinger Band width (volatility proxy)
      8  bb_position      – price position in BB channel 0=lower..1=upper
      9  atr_pct          – ATR as % of price
      10 atr_percentile   – ATR rank vs recent history 0=calm..1=spike
      11 regime           – market regime encoded
      12 side             – trade direction encoded
      13 rule             – entry rule encoded
      14 coin_enc         – coin encoded
      15 galaxy           – LunarCrush galaxy score (0 if no sentiment)
      16 sentiment        – LunarCrush sentiment % (50 if no sentiment)
      17 alt_rank_norm    – normalised alt rank (0 if no sentiment)
      18 has_sentiment    – 1 if real sentiment data, 0 if backtest placeholder
      19 funding_rate     – perp 8h funding rate (+ = longs paying = crowded long)
      20 fear_greed       – Alternative.me F&G 0-100 (50 = neutral)
    """
    ...
    bb_position = float(row.get("bb_position", 0.5))  # neutral if missing
    atr_pct = float(row.get("atr_pct", 0.01))
    atr_percentile = float(row.get("atr_percentile", 0.5))  # neutral if missing
    ...
    has_sentiment = float(row.get("has_sentiment", 0))
    funding_rate = float(row.get("funding_rate", 0))     # neutral = 0 (no perp bias)
    fear_greed = float(row.get("fear_greed", 50))         # neutral = 50
