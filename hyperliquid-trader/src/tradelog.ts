 ...
 * Also ensures the agent appears in the users table so it shows up
 * in the paper-trading dashboard user selector (even with 0 trades).
 */
export async function registerSession(opts: {
  ...
  try {
    await fetch(`${API_URL}/api/sessions`, {
      ...
      body: JSON.stringify({
        ...
        operator: process.env.TRADER_OPERATOR ?? null,
      }),
    });
  } catch {
  ...
  // Register in users table so agent appears in the paper trading user selector
  try {
    await fetch(`${API_URL}/api/users`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: opts.sessionId }),
    });
  } catch {
    /* non-fatal */
  }
 ...
 * Call every few cycles so the dashboard can show which agents are live.
 */
export async function updateSessionHeartbeat(sessionId: string): Promise<void> {
  try {
    await fetch(`${API_URL}/api/sessions/${sessionId}/heartbeat`, { method: "POST" });
  } catch {
    /* non-fatal */
  }
}
...
/**
 * Log a near-miss signal (entry that was blocked by confidence/ML/circuit-breaker).
 * Used by the Near Miss Analysis page.
 */
export async function logNearMiss(opts: {
  sessionId: string;
  coin: string;
  side: "long" | "short";
  rule: string;
  ...
  try {
    await fetch(`${API_URL}/api/v2/near-misses`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
