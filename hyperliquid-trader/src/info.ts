 ...
 * 1. HYPERLIQUID_ACCOUNT_ADDRESS if set
 * 2. Address derived from HYPERLIQUID_PRIVATE_KEY (single-wallet mode)
 * 3. null if neither configured
 */
export function getAccountAddressForReadOnly(): `0x${string}` | null {
  ...
  const keyRaw = process.env[KEY_ENV]?.trim();
