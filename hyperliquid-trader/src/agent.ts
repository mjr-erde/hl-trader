...
const accountAddress = (opts.account ?? process.env[opts.accountEnv ?? DEFAULT_ACCOUNT_ENV] ?? wallet.address) as `0x${string}`;
...
async function runDoctor(isStartup: boolean): Promise<DoctorResult[]> {
  ...
  if (!process.env.HYPERLIQUID_PRIVATE_KEY) {
    results.push({ check: "Exchange auth", status: "fail", message: "Internal auth error — check config" });
    writeLog("[DOCTOR DETAIL] HYPERLIQUID_PRIVATE_KEY not set in .env");
  } else {
  ...
  if (!process.env.NTFY_TOKEN) {
    results.push({ check: "Notifications auth", status: "warn", message: "Internal auth error — notifications degraded" });
    writeLog("[DOCTOR DETAIL] NTFY_TOKEN not set in .env");
  } else {
    results.push({ check: "Notifications auth", status: "ok" });
  }
  ...
  try {
    if (!existsSync(MODEL_FILE)) {
      results.push({ check: "ML: Model", status: "warn", message: "Model file missing — run: npx tsx hyperliquid-trader/src/backtest-export.ts && scorer.py --mode train" });
    } else {
      mlModelPresent = true;
  ...
  if (mlModelPresent && isStartup) {
    ...
    results.push({ check: "ML: Scorer", status: "warn", message: "Skipped — model not present" });
  }
  ...
  if (isStartup) {
    ...
    if (delta > 0.0001) {
      ...
      results.push({ check: "ML: Blend weight", status: "warn", message: "0 samples — ML has no influence (pure rule confidence). Train model to enable blending." });
    } else {
  ...
  try {
    if (!existsSync(LIVE_TRAIN_PATH)) {
      results.push({ check: "ML: Live data", status: "warn", message: `No live training data yet (${LIVE_TRAIN_PATH.split("/").pop()} missing) — will populate as trades close` });
    } else {
...
function printHelpBanner() {
  ...
  console.log(`  Notify ntfy.sh/${process.env.NTFY_CHANNEL ?? "not-configured"}`);
  console.log(`  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`);
}
