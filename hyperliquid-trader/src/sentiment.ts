/**
 * LunarCrush sentiment module + secondary sentiment sources.
 *
 * Primary: LunarCrush — social sentiment per coin (galaxyScore, sentiment%, altRank)
 * Secondary:
 *   - Alternative.me Fear & Greed Index — overall market psychology (free, no key)
 *   - Funding rates — market positioning signal (integrated by caller via exchange.ts)
 *
...
export interface SentimentSnapshot {
  ...
  // Secondary sentiment
  fundingRate?: number;       // perpetual 8h funding rate (+ = longs paying = crowded long)
}

/**
 * Market-wide Fear & Greed reading from Alternative.me.
 * Single value covers the entire crypto market — not per-coin.
 *
 * Interpretation:
 *   0-24   Extreme Fear   → contrarian buy zone
 *  25-49   Fear           → market anxious, longs may be closing
 *  50-74   Greed          → bullish bias, run trends
 *  75-100  Extreme Greed  → contrarian short zone (fade euphoria)
 */
 ...
 *
 * Free API, no key required. Data updates once per day.
 * Safe to call every cycle — cached by the server and rate-limited gently.
 *
 * Cross-reference logic:
 *   LC bullish + F&G > 75  → both agree on bullish, normal confidence
 *   LC bearish + F&G > 75  → extreme greed + LC bearish = strong contrarian short signal
 *   LC bullish + F&G < 25  → extreme fear + LC bullish = strong contrarian long signal
 *   LC bearish + F&G < 25  → both agree on bearish / fear
 */
export async function fetchFearGreed(): Promise<FearGreedReading | null> {
  try {
    ...
    const json = await resp.json() as { data?: Array<{ value: string; value_classification: string }> };
...
/**
 * Returns a short signal description for the Fear & Greed value.
 * Used in notifications and logs.
 */
export function fearGreedSignal(reading: FearGreedReading): "bullish_bias" | "bearish_bias" | "neutral" {
  if (reading.value >= 75) return "bearish_bias";  // extreme greed → fade
  if (reading.value <= 25) return "bullish_bias";  // extreme fear → buy
  return "neutral";
}

/**
 * Format Fear & Greed for display in logs/notifications.
 */
export function formatFearGreed(reading: FearGreedReading | null): string {
  if (!reading) return "F&G: unavailable";
  const emoji = reading.value >= 75 ? "🔴" : reading.value <= 25 ? "🟢" : "🟡";
  return `F&G: ${reading.value}/100 ${emoji} ${reading.classification}`;
}
