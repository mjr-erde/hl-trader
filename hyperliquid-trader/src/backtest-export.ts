...
async function main() {
  ...
  for (const coin of COINS) {
    ...
    try {
      ...
      let openAtrHistory: number[] = [];
      let coinTrades = 0;

      // Rolling ATR history (last 20 readings) for atr_percentile computation
      ...
      const buildRecord = (
        ...
        const atrPercentile = history.length >= 5
          ? history.filter((h) => h <= ind.atr).length / history.length
          : 0.5;
        return {
          coin,
          side,
          ...
          regime: ind.regime,
          // No real sentiment in backtest — use neutral placeholder
          // has_sentiment=0 tells the model to ignore these values
          galaxy_score: 0,
          sentiment_pct: 50,
          alt_rank: 500,
          has_sentiment: 0,
      ...
      for (let i = 50; i < candles.length - 12; i++) {
        // Update rolling ATR history for percentile computation
        const indForAtr = computeIndAt(candles, i);
        if (indForAtr) {
          atrHistory.push(indForAtr.atr);
        ...
        if (openBar !== null && openSide !== null && openRule !== null && openPrice !== null && openInd !== null) {
          ...
          if (i >= openBar + Math.max(1, exit.exitBar)) {
            allRecords.push(buildRecord(openInd, openSide, openRule, exit.won, exit.pnl, openAtrHistory));
            coinTrades++;
            openBar = null;
            openSide = null;
            openRule = null;
        ...
        // Open position — snapshot ATR history at entry
        openBar = i;
        openSide = signal.side;
        openRule = signal.rule;
        openPrice = ind.price;
        openInd = ind;
        openAtrHistory = [...atrHistory];
      }
      ...
      if (openBar !== null && openSide !== null && openRule !== null && openPrice !== null && openInd !== null) {
        ...
        allRecords.push(buildRecord(openInd, openSide, openRule, pnlPct >= 0, pnlPct * openPrice, openAtrHistory));
        coinTrades++;
      }
